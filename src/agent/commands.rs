...
impl Agent {
    ...
    pub(super) async fn handle_reasoning_command(
        ...
        let turns_to_show: Vec<&crate::agent::session::Turn> = match args.first().map(|s| s.as_str()) {
            ...
            None => {
                ...
                match thread.turns.iter().rev().find(|t| !t.tool_calls.is_empty()) {
                    Some(t) => vec![t],
                    None => return SubmissionResult::ok_with_message("No turns with tool calls."),
                }
            }
        };
        ...
        for turn in turns_to_show {
            ...
            if let Some(ref narrative) = turn.narrative {
